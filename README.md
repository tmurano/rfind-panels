...
Community reference registry for **RFind-sc** (Running Fisher for single-cell).

Each "reference" is a curated gene list (UP + optionally DOWN) that defines an
axis for scRNA-seq gene-set scoring. This registry holds references in a
...
### 1. Browse and try references in the browser

[**tmurano.github.io/rfind-panels**](https://tmurano.github.io/rfind-panels/)
is a static catalog of every reference in the registry. Pick any reference and
...
### 2. Submit your own reference (web form, no git)

Click **+ Contribute a reference** at the top of the catalog page. Drop a DEG
...
# Install a reference by its registry path
...
Or read a reference directly from the raw GitHub URL without any package:
...
aging <- read_reference("mouse/microglia/aging_hammond2019")
```

## Directory layout

```
<organism>/<tissue>/<reference_id>/
  ├── reference.yaml   # metadata (see schema.yaml)
  ├── up.tsv       # UP genes (gene/diff/rank)
  └── down.tsv     # DOWN genes (same format, absent for type=gene_set)
```

Index of all references: [registry.json](./registry.json)
Schema spec: [schema.yaml](./schema.yaml)

## Currently available references

| Reference ID | Organism | Tissue | Type | UP | DOWN | Source |
|---|---|---|---|---|---|---|
...
We welcome references for any organism, tissue, or biological condition.
...
Key rules (enforced by CI via `scripts/validate_reference.py`):
...
- Directory layout **must match** `<organism>/<tissue>/<id>/` and agree
  with the `organism` / `tissue` / `id` inside `reference.yaml`.
- `up.tsv` (and `down.tsv` for `type: deg`) header is literally
  `gene\tdiff\trank`.
- `n_up` / `n_down` in `reference.yaml` must equal the TSV row counts.
...
Reference metadata and curation scripts: [MIT](./LICENSE).
Individual gene lists retain the copyright/license of their original source (cited in `reference.yaml`).

## Citation

If you use references from this registry in a publication, please cite:

1. The original source paper (listed in each reference's `reference.yaml`).
